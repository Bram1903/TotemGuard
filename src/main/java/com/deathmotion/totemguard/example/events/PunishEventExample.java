...
public class PunishEventExample implements Listener {
    private final TotemGuardApiExample plugin;

    public PunishEventExample(TotemGuardApiExample plugin) {
        this.plugin = plugin;
    }

    @EventHandler
    public void onPunishEvent(PunishEvent event) {
        plugin.getLogger().info("Punish event triggered for player " + event.getTotemUser().getName() + " for check " + event.getCheck().getCheckName());
    }
}