...
public record ResolverContext(
        ...
        LoaderPaths paths,
        @Nullable FleetCacheRef fleetCacheRef
) {

    public static ResolverContext of(LoaderConfig config, HostPlatform platform, LoaderPaths paths,
                                     @Nullable FleetCacheRef fleetCacheRef) {
        return new ResolverContext(config, platform, paths, fleetCacheRef);
    }
}
