...
public class ApiDisabledEventTest implements Listener {
    private final ApiTestPlugin plugin;

    public ApiDisabledEventTest(ApiTestPlugin plugin) {
        this.plugin = plugin;
    }

    @EventHandler
    public void onApiDisable(ApiDisabledEvent event) {
        plugin.getLogger().info("API disabled!");
    }
}
