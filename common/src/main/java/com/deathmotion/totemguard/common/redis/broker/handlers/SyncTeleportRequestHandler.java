...
public final class SyncTeleportRequestHandler implements PacketProcessor<SyncTeleportRequestPacket.Payload>, Reloadable {

    private final TGPlatform platform;
    private final RedisRepositoryImpl redisRepository;
    private final PacketRegistry registry;
    private final Packet<SyncTeleportRequestPacket.Payload> packet;

    public SyncTeleportRequestHandler(TGPlatform platform, RedisRepositoryImpl redisRepository, PacketRegistry registry) {
        this.platform = platform;
        this.redisRepository = redisRepository;
        this.registry = registry;
        this.packet = Packets.SYNC_TELEPORT_REQUEST.packet();
    }

    @Override
    public void handle(SyncTeleportRequestPacket.Payload payload) {
        NetworkPresenceRepository presence = platform.getNetworkPresenceRepository();
        if (presence == null) return;
        presence.acceptTeleportRequest(payload);
    }
