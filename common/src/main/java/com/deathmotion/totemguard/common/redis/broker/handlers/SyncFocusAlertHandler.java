...
public final class SyncFocusAlertHandler implements PacketProcessor<SyncFocusAlertPacket.Payload>, Reloadable {

    private final TGPlatform platform;
    private final RedisRepositoryImpl redisRepository;
    private final PacketRegistry registry;
    private final Packet<SyncFocusAlertPacket.Payload> packet;

    public SyncFocusAlertHandler(TGPlatform platform, RedisRepositoryImpl redisRepository, PacketRegistry registry) {
        this.platform = platform;
        this.redisRepository = redisRepository;
        this.registry = registry;
        this.packet = Packets.SYNC_FOCUS_ALERT.packet();
    }

    @Override
    public void handle(SyncFocusAlertPacket.Payload payload) {
        if (platform.getAlertRepository() == null) {
            return;
        }

        platform.getAlertRepository().acceptRemoteFocusAlert(payload);
    }

    @Override
    public void reload() {
        registry.unregister(packet, this);
        if (!redisRepository.isEnabled()) {
            return;
        }
