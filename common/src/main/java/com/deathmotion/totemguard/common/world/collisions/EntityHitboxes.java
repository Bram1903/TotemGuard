...
public final class EntityHitboxes {
    ...
    private static final Map<EntityType, Size> SIZES = new HashMap<>(256);
    ...
    private static void put(EntityType type, double width, double height) {
        SIZES.put(type, new Size(width, height));
    }

    public static double width(EntityType type) {
        Size size = type == null ? null : SIZES.get(type);
        return size == null ? DEFAULT_WIDTH : size.width();
    }
