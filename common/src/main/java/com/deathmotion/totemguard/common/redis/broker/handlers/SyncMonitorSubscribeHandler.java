...
public final class SyncMonitorSubscribeHandler implements PacketProcessor<SyncMonitorSubscribePacket.Payload>, Reloadable {

    private final TGPlatform platform;
    private final RedisRepositoryImpl redisRepository;
    private final PacketRegistry registry;
    private final Packet<SyncMonitorSubscribePacket.Payload> packet;

    public SyncMonitorSubscribeHandler(TGPlatform platform, RedisRepositoryImpl redisRepository, PacketRegistry registry) {
        this.platform = platform;
        this.redisRepository = redisRepository;
        this.registry = registry;
        this.packet = Packets.SYNC_MONITOR_SUBSCRIBE.packet();
    }

    @Override
    public void handle(SyncMonitorSubscribePacket.Payload payload) {
        MonitorRepository monitor = platform.getMonitorRepository();
        if (monitor == null) return;
        monitor.acceptSubscribe(payload);
    }
