...
public final class SyncMonitorUpdateHandler implements PacketProcessor<MonitorSnapshot>, Reloadable {

    private final TGPlatform platform;
    private final RedisRepositoryImpl redisRepository;
    private final PacketRegistry registry;
    private final Packet<MonitorSnapshot> packet;

    public SyncMonitorUpdateHandler(TGPlatform platform, RedisRepositoryImpl redisRepository, PacketRegistry registry) {
        this.platform = platform;
        this.redisRepository = redisRepository;
        this.registry = registry;
        this.packet = Packets.SYNC_MONITOR_UPDATE.packet();
    }

    @Override
    public void handle(MonitorSnapshot snapshot) {
        MonitorRepository monitor = platform.getMonitorRepository();
        if (monitor == null) return;
        monitor.acceptUpdate(snapshot);
    }
