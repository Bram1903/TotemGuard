...
@CheckData(description = "Invalid sprint state transition", type = CheckType.PROTOCOL, experimental = true)
public class ProtocolF extends CheckImpl implements PacketCheck {
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        ...
        if (data.isRedundantSprint()) {
            fail("action={0}", action == WrapperPlayClientEntityAction.Action.START_SPRINTING ? "start" : "stop");
        }
    }
}
