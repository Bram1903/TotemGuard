...
public final class SyncTeleportRequestPacket extends Packet<SyncTeleportRequestPacket.Payload> {

    public SyncTeleportRequestPacket(int id) {
    ...
    public Payload read(ByteArrayDataInput input) {
        ...
        UUID subjectUuid = PacketIO.readUUID(input);
        ...
        return new Payload(requestId, subjectUuid, targetUuid, targetServerName, targetServerInstance, expiresAt);
    }

    @Override
    public void writeData(ByteArrayDataOutput output, Payload payload) {
        PacketIO.writeUUID(output, payload.requestId);
        PacketIO.writeUUID(output, payload.subjectUuid);
