...
public final class SyncPlayerJoinHandler implements PacketProcessor<SyncPlayerJoinPacket.Payload>, Reloadable {

    private final TGPlatform platform;
    private final RedisRepositoryImpl redisRepository;
    private final PacketRegistry registry;
    private final Packet<SyncPlayerJoinPacket.Payload> packet;

    public SyncPlayerJoinHandler(TGPlatform platform, RedisRepositoryImpl redisRepository, PacketRegistry registry) {
        this.platform = platform;
        this.redisRepository = redisRepository;
        this.registry = registry;
        this.packet = Packets.SYNC_PLAYER_JOIN.packet();
    }

    @Override
    public void handle(SyncPlayerJoinPacket.Payload payload) {
        NetworkPresenceRepository presence = platform.getNetworkPresenceRepository();
        if (presence == null) return;
        presence.acceptPlayerJoin(payload);
    }
