...
public final class SyncFollowEventHandler implements PacketProcessor<SyncFollowEventPacket.Payload>, Reloadable {

    private final TGPlatform platform;
    private final RedisRepositoryImpl redisRepository;
    private final PacketRegistry registry;
    private final Packet<SyncFollowEventPacket.Payload> packet;

    public SyncFollowEventHandler(TGPlatform platform, RedisRepositoryImpl redisRepository, PacketRegistry registry) {
        this.platform = platform;
        this.redisRepository = redisRepository;
        this.registry = registry;
        this.packet = Packets.SYNC_FOLLOW_EVENT.packet();
    }

    @Override
    public void handle(SyncFollowEventPacket.Payload payload) {
        FollowRepository repository = platform.getFollowRepository();
        if (repository == null) return;
        repository.acceptRemote(payload);
    }
