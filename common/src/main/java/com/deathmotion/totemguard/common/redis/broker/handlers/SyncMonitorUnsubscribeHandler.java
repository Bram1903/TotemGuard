...
public final class SyncMonitorUnsubscribeHandler implements PacketProcessor<SyncMonitorUnsubscribePacket.Payload>, Reloadable {

    private final TGPlatform platform;
    private final RedisRepositoryImpl redisRepository;
    private final PacketRegistry registry;
    private final Packet<SyncMonitorUnsubscribePacket.Payload> packet;

    public SyncMonitorUnsubscribeHandler(TGPlatform platform, RedisRepositoryImpl redisRepository, PacketRegistry registry) {
        this.platform = platform;
        this.redisRepository = redisRepository;
        this.registry = registry;
        this.packet = Packets.SYNC_MONITOR_UNSUBSCRIBE.packet();
    }

    @Override
    public void handle(SyncMonitorUnsubscribePacket.Payload payload) {
        MonitorRepository monitor = platform.getMonitorRepository();
        if (monitor == null) return;
        monitor.acceptUnsubscribe(payload);
    }
