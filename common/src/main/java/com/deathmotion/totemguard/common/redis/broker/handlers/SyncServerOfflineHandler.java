...
public final class SyncServerOfflineHandler implements PacketProcessor<SyncServerOfflinePacket.Payload>, Reloadable {

    private final TGPlatform platform;
    private final RedisRepositoryImpl redisRepository;
    private final PacketRegistry registry;
    private final Packet<SyncServerOfflinePacket.Payload> packet;

    public SyncServerOfflineHandler(TGPlatform platform, RedisRepositoryImpl redisRepository, PacketRegistry registry) {
        this.platform = platform;
        this.redisRepository = redisRepository;
        this.registry = registry;
        this.packet = Packets.SYNC_SERVER_OFFLINE.packet();
    }

    @Override
    public void handle(SyncServerOfflinePacket.Payload payload) {
        NetworkPresenceRepository presence = platform.getNetworkPresenceRepository();
        if (presence == null) return;
        presence.acceptServerOffline(payload);
    }
