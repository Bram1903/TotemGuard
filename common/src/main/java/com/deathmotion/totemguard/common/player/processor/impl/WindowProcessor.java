...
public class WindowProcessor extends PreProcessor {

    public WindowProcessor(TGPlayer player) {
        super(player);
    }
    ...
    public void handleOutgoing(PacketSendEvent event) {
        ...
        player.getPacketStateData().setServerOpenedInventoryThisTick(true);
    }

    @Override
    public void handleIncomingPost(PacketReceiveEvent event) {
        if (!player.isTickEndPacket(event.getPacketType())) return;
        player.getPacketStateData().setServerOpenedInventoryThisTick(false);
    }
}
