...
- `api/` — public API (`com.deathmotion.totemguard.api3.*`), published Maven artifact, compiled to Java 21, versioned independently (`1.0.0-SNAPSHOT`). Defines interfaces only: `TotemGuardAPI`, `EventRepository`, `ConfigRepository`, `UserRepository`, etc.
...
- `platforms/bukkit/` — Paper/Folia plugin (`TGBukkit` extends `JavaPlugin`, subclasses `TGBukkitPlatform extends TGPlatform`). Plugin name is `TotemGuardV3`; `folia-supported: true`; `load: POSTWORLD`; hard-depends on `packetevents`.
