    ...
    public @NotNull TGPluginHandle start(@NotNull TGPluginHost host) {
        ...
        if (!(native_ instanceof JavaPlugin javaPlugin)) {
            throw new IllegalStateException(
                    "TGBukkitEntry requires a JavaPlugin nativePlugin; got " + native_.getClass().getName());
        }
        ...
        return new BukkitHandle(platform);
    }

    private static final class BukkitHandle implements TGPluginHandle {

        private final TGBukkitPlatform platform;
        private volatile boolean stopped;

        BukkitHandle(TGBukkitPlatform platform) {
